...
def authenticate_gmail():
    ...
    # In GitHub Actions, try to load from environment variable first
    if os.getenv('GITHUB_ACTIONS') or os.getenv('CI'):
        ...
        if gmail_token:
            ...
            try:
                ...
                try:
                    import base64
                    token_data = base64.b64decode(gmail_token).decode('utf-8')
                except:
                ...
                with open('token.json', 'w') as f:
                    f.write(token_data)
                print("✅ Gmail token written to token.json from environment")
                
            except Exception as e:
                print(f"❌ Error processing GMAIL_TOKEN_JSON: {e}")
        else:
            print("⚠️ GMAIL_TOKEN_JSON environment variable not found")
    ...
    if os.getenv('GITHUB_ACTIONS') or os.getenv('CI'):
        ...
        print("❌ Solutions:")
        print("   1. Run script locally to generate token.json, then add as GMAIL_TOKEN_JSON secret")
        print("   2. Or commit token.json to repository (less secure)")
        print("   3. Or use service account authentication instead")
